...
class Stage extends FlxTypedGroup<FlxBasic> {
  public static var songStageMap:Map<String,String> = [
    ...
    "pending" =>"stage",
    "withheld"=>"stage",
    "polysaccharide-crush"=>"cafe",
    "under-the-groove"=>"cafe",
    "chromatic-breakdown"=>"cafe"
  ];
  ...
  public function new(stage:String,currentOptions:Options){
    ...
    switch (stage){
      ...
      case 'cafe':
        ...
        gfPosition.x -= 90;
        gfPosition.y += 30;
        var cafe:FlxSprite = new FlxSprite(-570, -200).loadGraphic(Paths.image('open','cafe'));
        cafe.antialiasing = true;
        cafe.setGraphicSize(Std.int(cafe.width*0.83));
        cafe.updateHitbox();
        add(cafe);

        var kit:FlxSprite = new FlxSprite(-562, -125).loadGraphic(Paths.image('kitchen', 'cafe'));
        kit.antialiasing = true;
        kit.setGraphicSize(Std.int(kit.width*0.83));
        kit.updateHitbox();
        kit.scrollFactor.set(0.9,0.8);
        add(kit);
