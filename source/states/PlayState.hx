...
class PlayState extends MusicBeatState
{
	...
	public var underColors:Array<FlxColor> = [
		0xffffffff,
		0xff000000,
		0xff5858f0,
		0xff4141ae,
	...
	function setupLuaSystem(){
		...
		if(luaModchartExists){
			...
			Lua_helper.add_callback(lua.state,"newCamera", function(?x:Int=0, ?y:Int=0,?cameraName:String){
				...
				modManager.defineMod(name, new modchart.modifiers.CamModifier(modManager, name, [cam]));
			...
			var luaSustainNotes = new LuaGroup<Note>(sustainNotes,"sustainNotes",true);
			...
			defaultLuaClasses = [luaModchart,window,bfLua,gfLua,dadLua,bfIcon,dadIcon,luaGameCam,luaHUDCam,luaNotesCam,luaSustainCam,luaReceptorCam,luaRenderedNotes, luaSustainNotes];
	...
	override public function create()
	{
		...
		if (currentOptions.betaCharacters) {if (SONG.player2 == "lcdy-new") SONG.player2 = "lcdy";}
		else {if (SONG.player2 == "lcdy") SONG.player2 = "lcdy-new";}
	...
	function startCountdown():Void
	{
		...
		inCutscene = false;
		#if cpp
		if(luaModchartExists && lua!=null) lua.call("startCountdown",[]);
		#end
		...
		if(inCharter){
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					FlxG.sound.music.loopTime=((60 / 128) * 1000) * 16;
					FlxG.sound.music.time=((60 / 128) * 1000) * 16;
					FlxG.sound.music.endTime=((60 / 128) * 1000) * 80;
