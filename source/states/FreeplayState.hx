...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		isDebug = true;
			...
			if (FlxG.sound.music != null && (isDebug || !OptionUtils.options.freeplayPreview))
			{
				if (!FlxG.sound.music.playing)
				{
					FlxG.sound.playMusic(Paths.music('freakyMenu'));
					if(!OptionUtils.options.freeplayPreview) Conductor.changeBPM(128);
	...
	function changeSelection(change:Int = 0,additive:Bool=true)
	{
		...
		if(OptionUtils.options.freeplayPreview){
			...
			Conductor.changeBPM(Song.loadFromJson(songs[curSelected].chartName,songs[curSelected].chartName).bpm);
		}
