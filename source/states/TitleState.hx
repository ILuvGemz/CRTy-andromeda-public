...
//			logoBl.updateHitbox();
			logoDill = new FlxSprite(-80, -72);
			logoDill.loadGraphic(Paths.image('D-illusions'));
			logoDill.antialiasing = true;
			logoDill.updateHitbox();
		}
		else
		{
		...
		credTextShit = new Alphabet(0, 0, "Zenokwei aka ILuvGemz\nNomDomeStudios\njarlungus\nFoguDragon", true);
	...
	override function update(elapsed:Float)
	{
		...
		if (pressedEnter && !transitioning && skippedIntro)
		{
			...
			new FlxTimer().start(2, function(tmr:FlxTimer)
			{
				...
				if (EngineData.options.loadModcharts) FlxG.switchState(new WarningSubState());
	...
	override function beatHit()
	{
		...
		//logoBl.animation.play('bump');
		logoDill.scale.set(0.77,0.77);
		FlxTween.tween(logoDill, {'scale.x': 0.6, 'scale.y': 0.6}, Conductor.stepCrochet * 3.5 / 1000, {ease: FlxEase.quadOut});
		...
		{
			case 1:
				createCoolText(['Zenokwei aka ILuvGemz', 'NomDomeStudios', 'jarlungus', 'FoguDragon']);
