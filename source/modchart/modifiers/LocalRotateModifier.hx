...
class LocalRotateModifier extends Modifier { // this'll be rotateX in ModManager
  ...
  override function getPath(visualDiff:Float, pos:Vector3, data:Int, player:Int, timeDiff:Float){
    ...
    diff.z *= scale;
    var out = rotateV3(diff, (getPercent(player)+getSubmodPercent('localrotate${data}X',player))*100, (getSubmodPercent('localrotateY',player)+getSubmodPercent('localrotate${data}Y',player))*100, (getSubmodPercent('localrotateZ',player)+getSubmodPercent('localrotate${data}Z',player))*100);
    out.z /= scale;
    return origin.add(out);
  }

  override function getSubmods(){
    var subs = ['localrotateY','localrotateZ'];
    for (i in 0...4)
    {
      subs.push('localrotate' + i + "X");
      subs.push('localrotate' + i + "Y");
      subs.push('localrotate' + i + "Z");
    }
    return subs;
  }
}
